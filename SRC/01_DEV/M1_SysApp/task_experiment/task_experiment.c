...
static uint32_t is_start_exp = 0;
static uint32_t mon_delay = 3600;
...
void Task_Experiment(void *pvParameters)
{
    ...
    // SET_UP_STATE:
        ...
        m33_get_is_start_exp(&is_start_exp);
    ...
    //     m33_get_get_mon_delay(&mon_delay);
    //     if (mon_delay != 0)
    //     {
    //         mon_delay--; // default 1 hour
    //         m33_get_set_mon_delay(mon_delay);
    //         continue;
    //     }
    //     else {
    //         m33_get_get_mon_interval(&mon_delay);
    //         m33_get_set_mon_delay(mon_delay);
    //     }
